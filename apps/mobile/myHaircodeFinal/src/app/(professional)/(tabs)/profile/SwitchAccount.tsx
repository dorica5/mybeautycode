...
const RETURN_TO_HREF: Record<string, Href> = {
  ...
  "pro-profile": "/(professional)/(tabs)/profile",
};
...
/** This screen lives under (professional)-stack → pathname does not tell if user is on client UI. */
...
const SwitchAccountScreen = () => {
  ...
  const handleBack = useCallback(() => {
    const rt = rawParamFirst(returnToParam);

    // Opening from hairdresser profile uses push onto this stack; pop gives the
    // correct “screen exits to the right” transition. replace() animates like a
    // forward navigation and feels inverted for “back”.
    if (rt === "pro-profile") {
      router.back();
      return;
    }

    const target = resolveReturnHref(rt, activeSurface);
  ...
  const onSelectRow = useCallback(
    async (row: AccountSurfaceRow) => {
      ...
      void queryClient.removeQueries({ queryKey: ["latest_visits"] });
      void queryClient.removeQueries({ queryKey: ["clientSearch"] });
      void queryClient.invalidateQueries({ queryKey: ["latest_visits"] });
