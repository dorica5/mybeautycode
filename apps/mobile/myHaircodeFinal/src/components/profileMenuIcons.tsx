...
/** Plus in circle (add account). */
export function ProfileMenuAddAccountIcon({
  size = 24,
  color = "#212427",
}: IconProps) {
  const dim = typeof size === "number" ? size : 24;
  const clipId = "profileMenuClipAddAccount";
  return (
    <Svg width={dim} height={dim} viewBox="0 0 24 24" fill="none">
      <Defs>
        <ClipPath id={clipId}>
...
/**
 * Filled dark circle with light plus — “Add account” on switch-account screen.
 * Matches design: circle fill #212427, plus strokes #F1F9F4 (primaryWhite).
 */
export function ProfileMenuSwitchAccountAddIcon({
  size = 24,
}: Pick<IconProps, "size">) {
  const dim = typeof size === "number" ? size : 24;
  const clipId = "profileMenuClipSwitchAdd";
  const fg = "#F1F9F4";
  const bg = "#212427";
  return (
    <Svg width={dim} height={dim} viewBox="0 0 24 24" fill="none">
