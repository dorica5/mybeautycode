...
export type ClientProfessionalProfileScreenProps = {
  ...
  /** Discover lane (map / search) — never infer from the pro's other accounts. */
  routeProfessionRaw?: string | null;
  relationshipFromRoute?: boolean;
  onBack: () => void;
...
export function ClientProfessionalProfileScreen({
  ...
  const relationshipLane = useMemo((): ProfessionChoiceCode | null => {
    const raw = routeProfessionRaw?.trim();
    if (!raw) return null;
  ...
  const isRelated =
    ...
    (relFetched
      ? isRelatedFromApi
      : isRelatedFromApi || relationshipFromRoute === true);
  const relationshipStatusLoading =
    !linkJustAdded &&
    !isRelatedFromApi &&
    relationshipFromRoute === undefined &&
  ...
  }, [addHairdresserDB, hairdresser_id, client_id, queryClient, relationshipLane, t]);
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      <PublicProfessionalProfileView
        ...
        hideColorBrand
        topInsetHandledExternally={topInsetHandledExternally}
        professionCodes={
